...
#endif
#if defined(AARCH64_CE_ASM) && defined(AARCH64_ASM) && HAVE_SYS_AUXV_H && HAVE_GETAUXVAL
# include <sys/auxv.h>
#endif
...
#if defined(AARCH64_CE_ASM) && defined(AARCH64_ASM) && HAVE_SYS_AUXV_H && HAVE_GETAUXVAL
static int aarch64_AES_CE_is_present(void)
{
    /*
     * This aarch64 AES opcode detection is non-portable and Linux only.
     * Reading system register ID_AA64ISAR0_EL1 is not available
     * to user space. getauxval() is a nonstandard glibc extension.
     */
    unsigned long hwcaps = getauxval(AT_HWCAP);
    if(hwcaps & HWCAP_AES) return 1;
    return 0;
}

static void aarch64_AES_CE_multikey_decrypt(int size)
{
    aes_context     *a;
    u_int64_t       *bfp = (u_int64_t *)bufb;

    while(size > 0) {
        a = multiKeyCtx[((unsigned)devSect0) & 0x3F];
        /* decrypt using fake all-zero IV */
        memset(ivbuf, 0, 16);
        _ARM_CE_aes_cbc_decrypt((unsigned char *)bfp, (unsigned char *)bfp, &a->aes_d_key[0], (int)a->aes_Nrnd, 32, (unsigned char *)ivbuf);
        /* compute correct IV */
        memcpy(ivbuf, &partialMD5[0], 16);
        compute_md5_iv_v3((u_int32_t *)ivbuf, (u_int32_t *)(bfp + 2));
        /* XOR with correct IV now */
...
static void aarch64_AES_CE_singlekey_decrypt(int size)
{
    int             x;
    u_int64_t       *bfp = (u_int64_t *)bufb;
    ...
    while(size > 0) {
        ...
        x = size;
        if(x > 512) x = 512;
         _ARM_CE_aes_cbc_decrypt((unsigned char *)bfp, (unsigned char *)bfp, &ctx->aes_d_key[0], (int)ctx->aes_Nrnd, x >> 4, (unsigned char *)ivbuf);
        size -= 512;
        bfp += 64;
    }
}

static void aarch64_AES_CE_multikey_encrypt(int size)
{
    aes_context     *a;
    u_int64_t       *bfp = (u_int64_t *)bufb;
    ...
    while(size > 0) {
        ...
        memcpy(ivbuf, &partialMD5[0], 16);
        compute_md5_iv_v3((u_int32_t *)ivbuf, (u_int32_t *)(bfp + 2));
        _ARM_CE_aes_cbc_encrypt((unsigned char *)bfp, (unsigned char *)bfp, &a->aes_e_key[0], (int)a->aes_Nrnd, 32, (unsigned char *)ivbuf);
        size -= 512;
        bfp += 64;
    }
}

static void aarch64_AES_CE_singlekey_encrypt(int size)
{
    int             x;
    u_int64_t       *bfp = (u_int64_t *)bufb;
    ...
    while(size > 0) {
        ...
        x = size;
        if(x > 512) x = 512;
        _ARM_CE_aes_cbc_encrypt((unsigned char *)bfp, (unsigned char *)bfp, &ctx->aes_e_key[0], (int)ctx->aes_Nrnd, x >> 4, (unsigned char *)ivbuf);
        size -= 512;
        bfp += 64;
    }
}

static void (*aarch64_AES_CE_workFunc[4])(int) = {
    aarch64_AES_CE_singlekey_decrypt,
    aarch64_AES_CE_singlekey_encrypt,
    aarch64_AES_CE_multikey_decrypt,
    aarch64_AES_CE_multikey_encrypt
};
#endif
...
#endif
#if defined(AARCH64_CE_ASM) && defined(AARCH64_ASM) && HAVE_SYS_AUXV_H && HAVE_GETAUXVAL
    if(aarch64_AES_CE_is_present() == 1) {
        workFunc = aarch64_AES_CE_workFunc[x];
        hardware = "aarch64 AES Crypto Extension";
    } else
#endif
    {
...
#elif defined(ARM32_ASM)
        hardware = "arm assembler AES";
#elif defined(AARCH64_ASM)
        hardware = "aarch64 assembler AES";
#else
